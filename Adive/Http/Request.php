...
class Request
{
    ...
    /**
     * Constructor
     * @param \Adive\Environment $env
     */
    public function __construct(\Adive\Environment $env)
    {
        $this->env = $env;
        $this->headers = new \Adive\Http\Headers(\Adive\Http\Headers::extract($env));
        $this->cookies = new \Adive\Security\Set(\Adive\Http\Util::parseCookieHeader($env['HTTP_COOKIE']));
    }
    ...
    {
        if (!isset($this->env['adive.request.query_hash'])) {
            $output = array();
            if (function_exists('mb_parse_str') && !isset($this->env['adive.tests.ignore_multibyte'])) {
                mb_parse_str($this->env['QUERY_STRING'], $output);
            } else {
                parse_str($this->env['QUERY_STRING'], $output);
            }
            $this->env['adive.request.query_hash'] = Util::stripSlashesIfMagicQuotes($output);
        }
        if ($key) {
            if (isset($this->env['adive.request.query_hash'][$key])) {
                return $this->env['adive.request.query_hash'][$key];
            } else {
                return $default;
            }
        } else {
            return $this->env['adive.request.query_hash'];
        }
    }
    ...
    {
        if (!isset($this->env['adive.input'])) {
            throw new \RuntimeException('Missing adive.input in environment variables');
        }
        if (!isset($this->env['adive.request.form_hash'])) {
            $this->env['adive.request.form_hash'] = array();
            if ($this->isFormData() && is_string($this->env['adive.input'])) {
                $output = array();
                if (function_exists('mb_parse_str') && !isset($this->env['adive.tests.ignore_multibyte'])) {
                    mb_parse_str($this->env['adive.input'], $output);
                } else {
                    parse_str($this->env['adive.input'], $output);
                }
                $this->env['adive.request.form_hash'] = Util::stripSlashesIfMagicQuotes($output);
            } else {
                $this->env['adive.request.form_hash'] = Util::stripSlashesIfMagicQuotes($_POST);
            }
        }
        if ($key) {
            if (isset($this->env['adive.request.form_hash'][$key])) {
                return $this->env['adive.request.form_hash'][$key];
            } else {
                return $default;
            }
        } else {
            return $this->env['adive.request.form_hash'];
        }
    }

    /**
     * Fetch PUT data (alias for \Adive\Http\Request::post)
     * @param  string           $key
     * @param  mixed            $default Default return value when key does not exist
     * @return array|mixed|null
     */
    ...
    /**
     * Fetch PATCH data (alias for \Adive\Http\Request::post)
     * @param  string           $key
     * @param  mixed            $default Default return value when key does not exist
     * @return array|mixed|null
     */
    ...
    /**
     * Fetch DELETE data (alias for \Adive\Http\Request::post)
     * @param  string           $key
     * @param  mixed            $default Default return value when key does not exist
     * @return array|mixed|null
     */
    ...
    public function cookies($key = null)
    {
        ...
        return $this->cookies;
        // if (!isset($this->env['adive.request.cookie_hash'])) {
        //     $cookieHeader = isset($this->env['COOKIE']) ? $this->env['COOKIE'] : '';
        //     $this->env['adive.request.cookie_hash'] = Util::parseCookieHeader($cookieHeader);
        // }
        // if ($key) {
        //     if (isset($this->env['adive.request.cookie_hash'][$key])) {
        //         return $this->env['adive.request.cookie_hash'][$key];
        //     } else {
        //         return null;
        //     }
        // } else {
        //     return $this->env['adive.request.cookie_hash'];
        // }
    }
    ...
    {
        $method = isset($this->env['adive.method_override.original_method']) ? $this->env['adive.method_override.original_method'] : $this->getMethod();
    ...
    {
        return $this->env['adive.url_scheme'];
    }
